...
def Bi_LSTM_D(_X, _weights, _biases, _batch_size, _seq_len, _n_input):
    ...
    o1 = hidden_layer(name="dh", X=outputs, weights=_weights['hidden'], biases=_biases['hidden'], n_input=2 * _n_input, seq_len=n_step)
...
def Bi_LSTM_Q(_X, _weights, _biases, _batch_size, _seq_len, _n_input):
    ...
    output1 = hidden_layer(name="qh1", X=output1, weights=_weights['hidden'], biases=_biases['hidden'], n_input=_n_input,
                           seq_len=2)
    output2 = hidden_layer(name="qh2", X=output2, weights=_weights['hidden'], biases=_biases['hidden'], n_input=_n_input,
                           seq_len=2)
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    ...
    for i in range(training_iters):
        ...
        while step < 100:
            ...
            sess.run(optimizer, feed_dict={d: batch_d, q: batch_d, a: batch_q,
            ...
            if step % display_step == 0:
                ...
                acc = sess.run(accuracy, feed_dict={d: batch_d, q: batch_d , a: batch_q,
                                           istate_fw_d: np.zeros((batch_size, 2 * n_hidden)),
                                           istate_bw_d: np.zeros((batch_size, 2 * n_hidden)),
                                           istate_fw_q: np.zeros((batch_size, 2 * n_hidden)),
                                           istate_bw_q: np.zeros((batch_size, 2 * n_hidden)),
                ...
                loss = sess.run(cost, feed_dict={d: batch_d, q: batch_d , a: batch_q,
                                           istate_fw_d: np.zeros((batch_size, 2 * n_hidden)),
                                           istate_bw_d: np.zeros((batch_size, 2 * n_hidden)),
                                           istate_fw_q: np.zeros((batch_size, 2 * n_hidden)),
                                           istate_bw_q: np.zeros((batch_size, 2 * n_hidden)),
            ...
            step += 1
        test_len = batch_size
        test_d = mnist.test.images[:test_len].reshape((-1, n_steps_d, n_input_d))
        test_a = mnist.test.labels[:test_len]
        print("Testing Accuracy:", sess.run(accuracy, feed_dict={d: test_d, q: test_d , a: test_a,
                                           istate_fw_d: np.zeros((batch_size, 2 * n_hidden)),
                                           istate_bw_d: np.zeros((batch_size, 2 * n_hidden)),
                                           istate_fw_q: np.zeros((batch_size, 2 * n_hidden)),
                                           istate_bw_q: np.zeros((batch_size, 2 * n_hidden)),
